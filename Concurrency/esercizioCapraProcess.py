...
def procTesto(file, stringhe):
    ...
    conc = len(file)
    create_process(jobs, result, conc)
    for i, x in enumerate(file):
        ...
        with open(x) as fp:
            lines = fp.readlines()
            for y in lines:
                stringhe_spezzate = y.split(" ")
                for z in stringhe_spezzate:
                    all_str_in_file.append(z)
        add_job(x, all_str_in_file, str, jobs)
    jobs.join()
    while not result.empty():
        print(result.get_nowait())


def add_job(file, all_str_in_file, str_to_find, jobs):
    jobs.put((file, all_str_in_file, str_to_find))


def create_process(jobs, result, concurrency=1):
...
def worker(jobs, result):
    while True:
        file, str_file, str_find = jobs.get()
        result.put(cerca_stringa(file, str_file, str_find))
        jobs.task_done()
